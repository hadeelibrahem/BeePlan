...
export class WhiteboardService {
  ...
  async listBoards(
    ...
    options: {
      archived?: boolean;
      search?: string;
      sort?: 'lastOpenedAt' | 'updatedAt' | 'name' | 'createdAt';
    } = {},
  ) {
    const boards = await this.whiteboardRepository.findAllAccessibleByUserId(
      userId,
      options,
    );
    return boards.map(({ board, accessRole }) =>
      this.toSummary(board, accessRole),
    );
  }
  ...
  async createBoard(userId: string, dto: CreateWhiteboardDto) {
    ...
    if (!board)
      throw new InternalServerErrorException('Board could not be created.');
    return this.toResponse(board);
  }

  async duplicateBoard(userId: string, id: string) {
    if (!this.whiteboardAssetsRepository || !this.databaseService)
      throw new InternalServerErrorException(
        'Board duplication is unavailable.',
      );
    ...
    const references = (source.assetReferences ?? {}) as Record<
      string,
      { beeplanAssetId?: string; stableResolverUrl?: string }
    >;
    const sourceAssetIds = [
      ...new Set(
        Object.values(references)
          .map((reference) => reference.beeplanAssetId)
          .filter((value): value is string => Boolean(value)),
      ),
    ];
    const sourceAssets = await this.whiteboardAssetsRepository.listForBoard(
      userId,
      source.id,
    );
    ...
    try {
      for (const [tldrawAssetId, reference] of Object.entries(references)) {
        if (!reference.beeplanAssetId)
          throw new BadRequestException(
            'Whiteboard asset reference is missing an asset ID.',
          );
        const sourceAsset = assetsById.get(reference.beeplanAssetId);
        if (
          !sourceAsset ||
          !sourceAsset.storagePath.startsWith(
            `${userId}${process.platform === 'win32' ? '\\' : '/'}`,
          )
        )
          throw new NotFoundException('Whiteboard asset not found.');
        ...
        remapped[tldrawAssetId] = {
          beeplanAssetId: newAssetId,
          stableResolverUrl: this.replaceAssetId(
            reference.stableResolverUrl,
            reference.beeplanAssetId,
            newAssetId,
          ),
        };
      }
      ...
      const board = await this.databaseService.db.transaction(async (tx) => {
        ...
        if (copiedAssets.length)
          await tx.insert(whiteboardAssets).values(copiedAssets);
        return created;
      });
      if (!board)
        throw new InternalServerErrorException(
          'Board could not be duplicated.',
        );
      return this.toResponse(board);
    } catch (error) {
  ...
  private async copyStoredFile(sourcePath: string, targetPath: string) {
    ...
    if (
      ...
      throw new BadRequestException('Invalid whiteboard storage path.');
  ...
  private async removeStoredFile(storagePath: string) {
    try {
      const root = join(process.cwd(), 'apps', 'api', 'uploads', 'whiteboards');
      await unlink(join(root, storagePath));
    } catch {
      /* rollback cleanup is best effort */
    }
  }

  private replaceAssetId(
    url: string | undefined,
    sourceId: string,
    targetId: string,
  ) {
    return typeof url === 'string' && url.includes(sourceId)
      ? url.replaceAll(sourceId, targetId)
      : `/whiteboard/assets/${targetId}/file`;
  }

  async updateBoard(
    _userId: string,
    id: string,
    dto: UpdateWhiteboardBoardDto,
    _accessRole = 'owner',
  ) {
    ...
    const board = await this.whiteboardRepository.updateById(id, {
      ...
      ...(dto.assetReferences !== undefined
        ? { assetReferences: dto.assetReferences }
        : {}),
      ...(dto.camera
        ? {
            cameraX: dto.camera.x,
            cameraY: dto.camera.y,
            cameraZoom: dto.camera.zoom,
          }
        : {}),
      ...(dto.isPinned !== undefined ? { isPinned: dto.isPinned } : {}),
    });
    if (!board)
      throw new InternalServerErrorException('Board could not be saved.');
    return this.toResponse(board);
  }
  ...
  async setArchived(userId: string, id: string, archived: boolean) {
    const board = await this.whiteboardRepository.updateByIdForUser(
      id,
      userId,
      { isArchived: archived },
    );
  ...
  async deleteBoard(userId: string, id: string) {
    const deleted = await this.whiteboardRepository.deleteByIdForUser(
      id,
      userId,
    );
  ...
  private toSummary(
    board: Awaited<ReturnType<WhiteboardRepository['findByIdForUser']>>,
    accessRole = 'owner',
  ) {
    if (!board)
      throw new InternalServerErrorException('Board could not be loaded.');
    return {
      id: board.id,
      name: board.name,
      previewUrl: board.previewUrl ?? null,
      isPinned: accessRole === 'owner' && board.isPinned,
  ...
  private toResponse(
    board: Awaited<ReturnType<WhiteboardRepository['findByUserId']>>,
    accessRole = 'owner',
  ) {
    if (!board) {
      throw new InternalServerErrorException('Whiteboard could not be loaded.');
