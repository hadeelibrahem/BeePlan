...
export class DashboardService {
  ...
  async getToday(userId: string) {
    ...
    const [user] = await this.db
      .select({ fullName: users.fullName, timezone: users.timezone })
      .from(users)
    ...
    const [taskRows, focusRows, recommendation, acceptedPlans] =
      await Promise.all([
        ...
        this.db
          ...
          .where(
            and(
              ...
              lt(focusSessions.startedAt, startOfTomorrow),
            ),
          ),
    ...
    const subtaskRows = taskIds.length
      ...
      : [];
    const focusToday = focusRows.map((session) =>
      this.toFocusEntity(session, taskRows, subtaskRows),
    );
    const todayCompletedFocusMinutes = focusRows
    ...
    const progress = progressForToday(
      [
        ...
        ...todaySubtasks.map((subtask) => ({
          dueAt: subtask.dueDate,
          status: subtask.status,
          estimatedMinutes: subtask.estimatedDurationMinutes ?? 0,
          spentMinutes: subtask.actualDurationMinutes ?? 0,
          completed: subtask.isDone || subtask.status === 'done',
        })),
      ],
      todayCompletedFocusMinutes,
    );
    const overdueCount = [...taskRows, ...subtaskRows].filter(
      (item) =>
        item.status !== 'done' &&
        item.status !== 'missed' &&
        item.dueDate &&
    ...
    const primary = activeFocus
      ? null
      : this.enrichRecommendation(recommendation, taskRows, subtaskRows);
    ...
    return {
      ...
      dailyStatus: {
        ...status,
        summaryLines: [
          progress.remainingEstimatedMinutes
            ? `${progress.remainingEstimatedMinutes} minutes of estimated work remain.`
            : 'No estimated work remains.',
          overdueCount
            ? `${overdueCount} overdue ${overdueCount === 1 ? 'item needs' : 'items need'} attention.`
            : `${tomorrowUnits.length} item${tomorrowUnits.length === 1 ? '' : 's'} due tomorrow.`,
        ],
      },
      activeFocus,
  ...
  private async getAcceptedPlans(
    userId: string,
    dates: string[],
  ): Promise<Map<string, any>> {
    const rows = await this.db
      .select({
        date: plannerAcceptedPlans.date,
        plan: plannerAcceptedPlans.plan,
      })
      .from(plannerAcceptedPlans)
      .where(
        and(
          eq(plannerAcceptedPlans.userId, userId),
          inArray(plannerAcceptedPlans.date, dates),
        ),
      );
    return new Map(rows.map((row) => [row.date, row.plan]));
  }

  private toFocusEntity(
  ...
  private enrichRecommendation(
    ...
    | (FocusRecommendation & { priority: string | null; dueAt: string | null })
    | null {
  ...
  private whyNow(
    ...
    if (due && due < now) reasons.push({ code: 'overdue', label: 'Overdue' });
      ...
      reasons.push({ code: 'due_today', label: 'Due today' });
    if (unit.priority === 'high' || unit.priority === 'urgent')
      reasons.push({ code: 'high_priority', label: 'High priority' });
    if (
      unit.status === 'in_progress' ||
      (!subtask && task && task.progress > 0)
    )
      reasons.push({ code: 'in_progress', label: 'Already in progress' });
    if (recommendation.estimatedMinutes && recommendation.estimatedMinutes > 0)
      reasons.push({
        code: 'estimated',
        label: 'Has an estimated focus length',
        value: `${recommendation.estimatedMinutes} min`,
      });
    return reasons;
  }
  ...
  private timeline(activeFocus: any, plan: any | null, date: string) {
    ...
    if (activeFocus)
      blocks.push({
        id: activeFocus.id,
        type: 'focus',
        startTime: activeFocus.startedAt,
        endTime: activeFocus.endedAt ?? null,
        title:
          activeFocus.subtaskTitle ?? activeFocus.taskTitle ?? 'Focus session',
        taskId: activeFocus.taskId,
        subtaskId: activeFocus.subtaskId,
        status: activeFocus.status,
        source: 'focus_session',
      });
    const sections =
      plan?.sections && typeof plan.sections === 'object'
        ? (Object.values(plan.sections).flat() as any[])
        : [];
    for (const item of sections) {
      ...
      blocks.push({
        id: item.id,
        type: item.isFocusTask
          ? 'focus'
          : item.type === 'calendar'
            ? 'event'
            : item.type,
        startTime: `${date}T${item.startTime}:00.000Z`,
        endTime: `${date}T${item.endTime}:00.000Z`,
        title: item.title,
        taskId: item.taskId ?? null,
        subtaskId: null,
        status: 'planned',
        source: 'accepted_daily_plan',
      });
    }
