...
export function WeatherTravelSettings({ token }: { token?: string }) {
  ...
  return <SectionCard>
    <h3 className="text-sm font-black text-[var(--bp-text)]">Weather &amp; Travel</h3>
    <p className="mb-4 text-xs text-[var(--bp-muted)]">BeePlan checks the expected route and forecast before scheduled tasks away from Home so it can suggest when to leave and what to prepare.</p>
    <label className="flex items-center justify-between gap-3 py-2 text-sm font-bold text-[var(--bp-text)]">Enable weather and travel advice<input aria-label="Enable weather and travel advice" type="checkbox" checked={value.enabled} onChange={(e) => set('enabled', e.target.checked)} /></label>
    <label className="block text-sm text-[var(--bp-text)]">Default travel mode<select className={input} value={value.defaultTravelMode} onChange={(e) => set('defaultTravelMode', e.target.value as any)}><option value="driving">Driving</option><option value="walking">Walking</option><option value="cycling">Cycling</option></select></label>
    <div className="mt-3 grid gap-3 sm:grid-cols-2">{numericFields.map(([key, label]) => <label key={key} className="text-xs font-bold text-[var(--bp-muted)]">{label}<input className={input} type="number" value={value[key] as number} onChange={(e) => set(key, Number(e.target.value) as never)} /></label>)}</div>
    <div className="mt-3 grid gap-2 sm:grid-cols-2">
      <Toggle label="Use current location fallback" value={value.currentLocationFallbackEnabled} onChange={(v) => set('currentLocationFallbackEnabled', v)} />
      <Toggle label="Allow approximate travel fallback" value={value.approximateTravelFallbackEnabled} onChange={(v) => set('approximateTravelFallbackEnabled', v)} />
      <Toggle label="AI message polishing" value={value.aiPolishingEnabled} onChange={(v) => set('aiPolishingEnabled', v)} />
      {adviceLabels.map(([key,label]) => <Toggle key={key} label={label} value={value.advice[key] !== false} onChange={(v) => set('advice', { ...value.advice, [key]: v })} />)}
    </div>
    <div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="text-xs font-bold text-[var(--bp-muted)]">Preferred language<select className={input} value={value.language} onChange={(e) => set('language', e.target.value)}><option value="en">English</option><option value="ar">العربية</option></select></label><label className="text-xs font-bold text-[var(--bp-muted)]">Timezone<input className={input} value={value.timezone} onChange={(e) => set('timezone', e.target.value)} /></label></div>
    <button type="button" onClick={() => void save()} className="mt-4 rounded-xl bg-[var(--bp-accent)] px-4 py-2 font-black text-[var(--bp-accent-text)]">Save Weather &amp; Travel</button><span aria-live="polite" className="ms-3 text-xs text-[var(--bp-muted)]">{status}</span>
  </SectionCard>
}
