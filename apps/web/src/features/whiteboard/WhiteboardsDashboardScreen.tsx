...
export function WhiteboardsDashboardScreen() {
  ...
  const load = () => {
    ...
    setLoading(true)
    void listWhiteboards(accessToken, { archived, search, sort: 'lastOpenedAt' }).then(setBoards).catch(() => setError('Unable to load whiteboards.')).finally(() => setLoading(false))
  }
  ...
  return <section className="flex min-h-0 flex-1 flex-col gap-6 py-4 text-[var(--bp-text)]">
    <header className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--bp-muted)]">Workspace</p><h1 className="mt-1 text-2xl font-bold">Whiteboards</h1></div><button type="button" disabled={creating} onClick={() => void create()} className="rounded-xl bg-[var(--bp-accent)] px-4 py-2 text-sm font-semibold text-[var(--bp-accent-text)]">{creating ? 'Creating...' : 'New Board'}</button></header>
    <div className="flex flex-wrap gap-3"><input aria-label="Search whiteboards" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search boards" className="min-w-56 flex-1 rounded-xl border border-[var(--bp-border)] bg-[var(--bp-surface)] px-3 py-2 text-sm outline-none" /><button type="button" onClick={() => setArchived((value) => !value)} className="rounded-xl border border-[var(--bp-border)] px-3 py-2 text-sm">{archived ? 'Archived' : 'Active'}</button></div>
    {error && <div className="rounded-xl border border-red-300 p-4 text-sm">{error}<button type="button" onClick={load} className="ms-3 font-semibold">Retry</button></div>}
    ...
    {!loading && !boards.length && <div className="rounded-2xl border border-dashed border-[var(--bp-border)] p-10 text-center"><h2 className="text-lg font-bold">{search ? 'No boards match your search.' : archived ? 'No archived boards.' : 'No whiteboards yet.'}</h2><p className="mt-2 text-sm text-[var(--bp-muted)]">{search ? 'Try a different search.' : 'Create a board for ideas, planning, tasks, notes, drawings, and files.'}</p>{!archived && !search && <button type="button" onClick={() => void create()} className="mt-5 rounded-xl bg-[var(--bp-accent)] px-4 py-2 text-sm font-semibold text-[var(--bp-accent-text)]">Create board</button>}</div>}
    ...
    {boards.length > 0 && <>
      {pinned.length > 0 && !archived && <BoardSection title="Pinned" boards={pinned} navigate={navigate} mutate={mutate} remove={remove} duplicate={duplicate} rename={rename} />}
      <BoardSection title={archived ? 'Archived' : 'My Whiteboards'} boards={boards.filter((board) => archived || !board.isShared)} navigate={navigate} mutate={mutate} remove={remove} duplicate={duplicate} rename={rename} />
      {!archived && <BoardSection title="Shared with Me" boards={boards.filter((board) => board.isShared)} navigate={navigate} mutate={mutate} remove={remove} duplicate={duplicate} rename={rename} />}
    </>}
  </section>
}

function BoardSection({ title, boards, navigate, mutate, remove, duplicate, rename }: { title: string; boards: WhiteboardSummary[]; navigate: ReturnType<typeof useNavigate>; mutate: (id: string, action: (token: string, id: string) => Promise<WhiteboardSummary>) => Promise<void>; remove: (board: WhiteboardSummary) => Promise<void>; duplicate: (board: WhiteboardSummary) => Promise<void>; rename: (board: WhiteboardSummary) => Promise<void> }) {
  return <div><h2 className="mb-3 text-sm font-bold">{title}</h2>{boards.length ? <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{boards.map((board) => <BoardCard key={board.id} board={board} onOpen={() => navigate(`/whiteboards/${board.id}`)} onPin={() => void mutate(board.id, board.isPinned ? unpinWhiteboard : pinWhiteboard)} onArchive={() => void mutate(board.id, board.isArchived ? restoreWhiteboard : archiveWhiteboard)} onDuplicate={() => duplicate(board)} onRename={() => rename(board)} onDelete={() => remove(board)} />)}</div> : <div className="rounded-xl border border-dashed border-[var(--bp-border)] px-4 py-5 text-sm text-[var(--bp-muted)]">{title === 'Shared with Me' ? 'No shared whiteboards.' : title === 'Archived' ? 'No archived whiteboards.' : 'No whiteboards yet.'}</div>}</div>
}
