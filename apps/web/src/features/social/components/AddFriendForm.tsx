...
export function AddFriendForm({ onAdd, onSearch }: Props) {
  ...
  const submit = async () => {
    ...
    setSuccess('')
    if (!valid) {
      setError(t('people.addFriend.invalidUsername'))
      return
    }
    if (onSearch && !match) {
      setLoading(true)
      try {
        ...
        if (!found) setError(t('people.friends.noMatch'))
        else setMatch(found)
  ...
  return (
    <section id="add-friend-card" className="border-b border-[var(--bp-border)] pb-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div><h3 className="text-sm font-bold text-[var(--bp-text)]">{t('people.addFriend.title')}</h3><p className="mt-0.5 text-xs text-[var(--bp-muted)]">{t('people.subtitle')}</p></div>
