...
export function EmptyState({ icon, illustration, title, description, actionLabel, onAction, variant = 'informational', size = 'md' }: EmptyStateProps) {
  ...
  return (
    <div data-empty-variant={variant} className={`flex flex-col items-center justify-center gap-3 rounded-2xl border border-[var(--bp-border)] bg-[var(--bp-surface)] px-4 text-center ${SIZE_CLASSES[size]}`}>
      {visual ? <div className={`flex h-12 w-12 items-center justify-center rounded-xl ${isFiltered ? 'bg-[var(--bp-border)] text-[var(--bp-muted)]' : 'bg-[var(--bp-accent)]/15 text-[var(--bp-accent-ink)]'}`}>{visual}</div> : null}
      <div>
      ...
      {!isFiltered && actionLabel && onAction && (
        <button
          type="button"
          onClick={onAction}
          className="rounded-lg border border-[var(--bp-accent)] bg-[var(--bp-accent-soft)] px-4 py-2 text-sm font-black text-[var(--bp-accent-text)] transition hover:brightness-95 active:scale-95"
        >
          {actionLabel}
        </button>
      )}
