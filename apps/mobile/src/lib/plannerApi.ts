...
export function generateDailyPlan(accessToken: string, date: string, lockedItems?: { taskId?: string; reminderId?: string; startTime: string; endTime: string }[]) { return request<DailyPlan>(accessToken, '/ai/planner/daily', { method: 'POST', body: JSON.stringify({ date, currentTime: new Date().toTimeString().slice(0, 5), lockedItems }) }) }
...
export function skipCommitmentOccurrence(accessToken: string, commitmentId: string, date: string) { return request<{ commitmentId: string; date: string; skipped: true }>(accessToken, `/context/commitments/${commitmentId}/skip`, { method: 'POST', body: JSON.stringify({ date }) }) }
export function resolveScheduleConflict(accessToken: string, payload: { conflictKey: string; date: string; taskId?: string; commitmentId: string; resolution: 'keep_commitment' | 'keep_task' | 'postpone_task' | 'cancel_task' }) { return request<{ conflictKey: string; lifecycle: 'resolved'; resolution: string }>(accessToken, '/ai/planner/conflicts/resolve', { method: 'POST', body: JSON.stringify(payload) }) }
