...
export function AccountPrivacySection({ accessToken, user, onUpdated }: Props) {
  ...
  useEffect(() => {
    ...
    void getInstallationId()
      .then((id) => getPushDeviceStatus(accessToken, id))
      .then((value) =>
        setPushStatus(
          value?.enabled
  ...
  async function savePassword() {
    if (
      ...
      return setMessage(
        "Enter the current password and matching valid new password.",
      );
    setBusy(true);
    setMessage("");
    try {
      await changePassword(accessToken, password.current, password.next);
  ...
  return (
    <View className="gap-4 px-4">
      <SectionCard>
        ...
        {(
          [
            [t('mobileSettings.profile'), "fullName"],
            [t('mobileSettings.profile'), "username"],
            [t('mobileSettings.profile'), "email"],
            [t('mobileSettings.profile'), "avatarUrl"],
            [t('mobileSettings.timezone'), "timezone"],
          ] as const
        ).map(([label, key]) => (
      ...
      {user.authProvider === "password" ? (
        <SectionCard>
          ...
          >
            ...
            >
              {t('mobileSettings.changePassword')}
            </Text>
          </Pressable>
        </SectionCard>
      ) : null}
      <SectionCard>
        <Text className="text-base font-black" style={{ color: colors.text }}>
          {t('mobileSettings.privacy')}
        </Text>
        <Row
          label={t('mobileSettings.location')}
          value={locationStatus}
          action={t('mobileSettings.openNotificationSettings')}
          onPress={() => void Linking.openSettings().then(refreshLocation)}
          colors={colors}
        />
        <Row
          label={t('mobileSettings.locationSharing')}
          value={t('mobileSettings.privacy')}
          colors={colors}
        />
        <Row label={t('mobileSettings.pushDevice')} value={pushStatus} colors={colors} />
      </SectionCard>
      {message ? (
        <Text
...
function Row({
  ...
  return (
    <View
      className="mt-3 flex-row items-center justify-between gap-3 border-b pb-3"
      style={{ borderColor: colors.border }}
    >
