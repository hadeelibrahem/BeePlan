...
export function SubtaskManagementSection({ task, accessToken, canEdit, onTaskUpdated, onItemsChange, onError }: Props) {
  ...
  return <View className="gap-2">
    ...
    {canEdit ? <Pressable onPress={() => setDraft({ title: '', description: '', isFocusTask: false })} className="rounded-xl border border-dashed py-3" style={{ borderColor: colors.border }}><Text className="text-center text-sm font-bold" style={{ color: colors.accentInk }}>+ Add Subtask</Text></Pressable> : null}
...
function FocusBadge() {
  const { theme } = useTheme()
  return <View className="rounded-full px-2 py-0.5" style={{ backgroundColor: `${theme.colors.accent}22` }}><Text className="text-[10px] font-black" style={{ color: theme.colors.accentInk }}>🎯 Focus</Text></View>
}
