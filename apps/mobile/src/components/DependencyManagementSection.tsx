...
export function DependencyManagementSection({ task, tasks, accessToken, canEdit, onTaskUpdated, onDependenciesChange, onError }: Props) {
  ...
  return <View className="gap-2">
    ...
    {canEdit ? <Pressable onPress={() => setSheet({ mode: 'add' })} className="rounded-xl border border-dashed py-3" style={{ borderColor: colors.border }}><Text className="text-center text-sm font-bold" style={{ color: colors.accentInk }}>+ Add Dependency</Text></Pressable> : null}
