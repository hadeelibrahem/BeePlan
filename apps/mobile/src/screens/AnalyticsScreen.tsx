...
export default function AnalyticsScreen({ accessToken, onBack }: { accessToken: string; onBack: () => void }) {
  ...
  return <AppScreen>
    ...
    {(tasksQuery.isError || remindersQuery.isError || focusQuery.isError) ? <View className="mb-3 rounded-xl p-3" style={{ backgroundColor: `${colors.error}18` }}><Text style={{ color: colors.error }}>Some analytics data is unavailable.</Text><Pressable onPress={retry} accessibilityRole="button" accessibilityLabel="Retry analytics"><Text className="mt-2 font-bold" style={{ color: colors.accentInk }}>Retry</Text></Pressable></View> : null}
